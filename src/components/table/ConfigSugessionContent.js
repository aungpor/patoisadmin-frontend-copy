...
import { getSugessionContent, getImageAndTitleByContentId, editContentSugession } from "../../services/content.service";
import './style.scss'
import 'antd/dist/antd.css';
import 'antd-button-color/dist/css/style.css';

function ConfigSugessionContent() {
    ...
    const fetchContentListData = async () => {
        const result = await getSugessionContent(tableParams)
    ...
    const onSave = async (data) => {
        try {
            ...
            const updateData = await editContentSugession(tempData)

            if (updateData.ResponseCode === 200) {
                notification.success({
                    message: 'Save sugession content successful'
                });
                fetchContentListData()
            ...
            notification.error({
                message: 'Failed save sugession content'
            });
        }
    }
                            ...
                            <Form onFinish={onSave} form={form}>
                                ...
                                <Form.Item>
                                    <Col span={12} offset={1} className="mb-24 margin-top-20px">
                                        <Button size={"large"} disabled={disableSaveBtn} type="success" className="btn-save-padding-lr-35px" onClick={() => form.submit()}>Save</Button>
                                    </Col>
                                </Form.Item>
                            </Form>
                        </div>
...
export default ConfigSugessionContent;
